                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4 text-center">
                            @if($project->image)
                                <img src="{{ asset($project->image) }}" alt="{{ $project->project_name }}" class="img-fluid img-thumbnail" style="max-height: 300px;">
                            @else
                                <img src="{{ asset('images/project-placeholder.png') }}" alt="صورة افتراضية" class="img-fluid img-thumbnail" style="max-height: 300px;">
                            @endif
                        </div>
                        <div class="col-md-8">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <table class="table table-bordered">
                                        ...
                                        <tr>
                                            <th>تاريخ الإضافة</th>
                                            <td>{{ $project->created_at ? date('Y-m-d', strtotime($project->created_at)) : 'غير متوفر' }}</td>
                                        </tr>
                                        <tr>
                                            <th>آخر تحديث</th>
                                            <td>{{ $project->updated_at ? date('Y-m-d', strtotime($project->updated_at)) : 'غير متوفر' }}</td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
                                                ...
                                                <tbody>
                                                    @foreach($requests as $request)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                @if($request->student)
                                                                    <a href="{{ route('students.show', $request->student->student_id) }}">
                                                            ...
                                                            <td>{{ $request->date_of_request ? date('Y-m-d', strtotime($request->date_of_request)) : 'غير متوفر' }}</td>
                                                            <td>
                                                            ...
                                                            <td>
                                                                <a href="{{ route('requests.show', $request->request_id) }}" class="btn btn-sm btn-info">
                    ...
                    <div class="row mt-4">
                        <div class="col-md-12 text-center">
                            <div class="btn-group">
                                ...
                                <a href="{{ route('requests.create') }}?project_id={{ $project->project_id }}&item_type=project" class="btn btn-success">
                                ...
                                <form action="{{ route('projects.destroy', $project->project_id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger" onclick="return confirm('هل أنت متأكد من حذف هذا المشروع؟')">
